...
@push('page-script')
    <script>
        ...
        function openGoogleLogin()
        {
            ...
            popupWindow = window.open('{{ route('auth.google') }}', '_blank', `width=${width},height=${height},left=${left},top=${top}`);
        ...
        async function checkPopupStatus()
        {
            if (!popupWindow || popupWindow.closed) {
                const user = await fetch('{{ route('auth.me') }}')
                user ? Livewire.navigate('{{ route('index') }}') : setTimeout(checkPopupStatus, 1000)
            } else {
